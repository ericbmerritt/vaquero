...
defmodule HandlerTest do
  ...
  test "assert delete with options and vars" do
    ...
    {:ok, _, :state} = REST1.handle_delete(req1, :state, <<"application/json">>)
    assert "{\"ok\":{\"got_bar\":\"bar\"}}" == :erlang.get(:body)

    {:ok, _, :state} = REST1.handle_delete(req2, :state, <<"application/json">>)
  ...
  test "complete handler" do
    ...
    {:ok, _, :state} = REST2.handle_delete(req0, :state, "application/json")
    assert "{\"ok\":\"ok\"}" == :erlang.get(:body)

    req1 = :cowboy_req.set_bindings([], ["path", "to", "resource", "bar"],
                                    [{:resource, "resource"}], req0)
    {:ok, _, :state} = REST2.handle_post(req1, :state, "application/json")
    ...
    req1 = :cowboy_req.set_bindings([], ["path", "to", "bar"],
                                    [{:resource, "bar"}], req0)
    {:ok, _, _} = REST2.handle_get(req1, :state, "application/json")
    assert "{\"json_get\":\"bar\"}" == :erlang.get(:body)
    {:ok, _, _} = REST2.handle_get(req1, :state, "text/html")
    assert "html_get: bar" == :erlang.get(:body)
    {:ok, _, _} = REST2.handle_get(req1, :state, "application/vaquero")
    ...
    {:ok, _, :state} = REST2.handle_head(req1, :state, "application/json")
    assert "" == :erlang.get(:body)
    {:ok, _, :state} = REST2.handle_head(req1, :state, "text/html")
    assert "" == :erlang.get(:body)
    ...
    {:ok, _, :state} = REST2.handle_put(req1, :state, "application/json")
    ...
    req1 = :cowboy_req.set_bindings([], ["path", "to", "bar"],
                                    [{:resource, "baz"}], req0)
    {:ok, _, :state} = REST2.handle_patch(req1, :state, "application/json")
    ...
    {:ok, _, :state} = REST2.handle_options(req1, :state, "application/json")
